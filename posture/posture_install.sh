...
LOGFNAME=`/bin/date "+posture-4.6.03049-install-%H%M%S%d%m%Y.log"` ||
  LOGFNAME=`/usr/bin/date "+posture-4.6.03049-install-%H%M%S%d%m%Y.log"` ||
  LOGFNAME=`date "+posture-4.6.03049-install-%H%M%S%d%m%Y.log"` ||
  LOGFNAME=posture-4.6.03049-install.log
...
echo "Installing Cisco AnyConnect Posture Module v4.6.03049..." > /tmp/${LOGFNAME}
